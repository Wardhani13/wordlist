...
def detail(keyword):
    ...
    if not definitions:
        return render_template('error.html', error_msg=f'Your word, "{keyword}", could not be found')

    if type(definitions[0]) is str:
        suggested_words = [word for word in definitions if word != keyword]
        return render_template('error.html', error_msg=f'Your word, "{keyword}", could not be found', suggested_words=suggested_words)
